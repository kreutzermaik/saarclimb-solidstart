export default class Toast {
    ...
    push(obj: any) {
        this.numToasts++;
        let toast: any = document.createElement(obj.link ? 'a' : 'div');
        if (obj.link) {
            toast.href = obj.link;
        ...
        if (toast.querySelector('.toast-notification-close')) {
            toast.querySelector('.toast-notification-close').onclick = (event: any) => {
    ...
    openToast(toast: any) {
        ...
        toast.element.addEventListener('transitionend', () => {
            if (toast.element.dataset.transitionState == 'opening') {
                ...
                if (toast.props.duration) {
                    setTimeout(() => {
                        this.closeToast(toast, toast.props.duration)
                    }, toast.props.duration);
                }
            }
        });
        for (let i = 0; i < this.stack.length; i++) {
    ...
    closeToast(toast: any, delay: any = null) {
        if (this.isOpening()) {
